...
def all_prof_drawd(cur_pairs, cash, commission, oround):
    all_profit_list = []
    all_drawdown_list = []
...
def draw_trading(cur_pairs, cash, oround, commission, ordparams, max_med, min_med):
    ldiff_koeff = 0.3
    while ldiff_koeff < 2:
        ldiff_koeff += 0.01
        for cur_pair in cur_pairs:
            if cur_pair == 'AUDJPY' or cur_pair == 'CADJPY' or cur_pair == 'CHFJPY' or cur_pair == 'EURJPY' or \
                    cur_pair == 'GBPJPY' or cur_pair == 'NZDJPY' or cur_pair == 'USDJPY':
                oround = 3
                ordparams = [ordparams[0], ordparams[1], ordparams[2], ordparams[3], ordparams[4] * 0.01]
            print(f" -> Торговля на {cur_pair}")
            download = Download(cur_pair)
                ...
                if find_max:
                    if high > maxi:
                        ...
                        try:
                            ...
                            print(" WARNING: Сделка на покупку не открыта! Прошлый минимум не найден")
                        find_min = True
                        find_max = False
                if find_min:
                    if low < mini:
                        ...
                        try:
                            ...
                            print(f" WARNING: Сделка на продажу не открыта! Прошлый максимум не найден")
                        find_min = False
                        find_max = True
            ...
            if flag and curcash[len(curcash) - 1] > 1020:
                ymax = []
                ymin = []
                time = []
                ...
                for i in range(len(good_sell)):
                    ...
                    # удачная продажа
                    if once:
                        ax1.plot(point2, point1, color='red', linewidth=2, label=f"Good sell")
                        once = False
                    else:
                ...
                for i in range(len(bad_sell)):
                    ...
                    # неудачная продажа
                    if once:
                        ax1.plot(point2, point1, color='blue', linewidth=2, label=f"Bad sell")
                        once = False
                    else:
                ...
                for i in range(len(good_buy)):
                    ...
                    # удачная продажа
                    if once:
                        ax1.plot(point2, point1, color='green', linewidth=2, label=f"Good buy")
                        once = False
                    else:
                ...
                for i in range(len(bad_buy)):
                    ...
                    # неудачная продажа
                    if once:
                        ax1.plot(point2, point1, color='black', linewidth=2, label=f"Bad buy")
                        once = False
                    else:
